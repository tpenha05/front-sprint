...
def dashboards(cores_personalizadas, df_rupturas, df_desfechos, contagem_desfechos, lista_porcentagem, dados):
    ...
    voltar, logo = st.columns([0.42,0.58])
    with voltar:
        if st.button("Voltar"):
...
def pagina_partidas(partidas):
    ...
    for index, partida in df_filtrado.iterrows():
        ...
        with col2:
            # escudo1,jogo,escudo2 = st.columns([1,2.8,1])
            # with escudo1:
            #     if partida['clube'] == 'Palmeiras':
            #         image = Image.open('design/photos/palmeiras.png')
            #         st.image(image, use_column_width=True)
            # with jogo:
                st.subheader(f"{partida['clube']} {partida['resultado']} {partida['adversario']}")
            # with escudo2:
            #     if partida['adversario'] == 'Bragantino':
            #         image = Image.open('design/photos/braga.png')
            #         st.image(image, use_column_width=True)
        with col3:
